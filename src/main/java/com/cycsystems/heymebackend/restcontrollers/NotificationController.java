...
public class NotificationController {
	...
	@Autowired
	public NotificationController(INotificacionService notificacionService, IContactoService contactoService,
			IUsuarioService usuarioService, IParametroService parametroService,
			IPaqueteConsumoService paqueteConsumoService, IDetallePaqueteService detallePaqueteService,
			SMSServiceImpl smsService, MailServiceImpl mailService) {
	...
	public ListenableFuture<ResponseEntity<NotificacionResponse>> cancelNotificationDelivery(
		...
		if (input.getNotificacion() == null) {
			...
			Notificacion notificacion = this.notificacionService
					.findById(input.getNotificacion().getIdNotificaciones());

			if (notificacion != null && notificacion.getEstado().getIdEstadoNotificacion()
					.compareTo(Constants.ESTADO_NOTIFICACION_PROGRAMADA) == 0) {

				notificacion.setEstado(new com.cycsystems.heymebackend.models.entity.EstadoNotificacion(
			...
			} else if (notificacion != null && notificacion.getEstado().getIdEstadoNotificacion()
					.compareTo(Constants.ESTADO_NOTIFICACION_CANCELADA) != 0) {
			...
			if (usuario == null) {
				...
				if (notificacion == null) {
					...
					List<PaqueteConsumo> listaPaquetes = this.paqueteConsumoService.findPackagesByCompanyAndStatus(
							usuario.getEmpresa().getIdEmpresa(), Constants.ESTADO_PAQUETE_CONSUMO_ACTIVO);
					...
					if (listaPaquetes == null || listaPaquetes.isEmpty()) {
						...
						String codigo = "";
						this.parametroService.findParameterByEmpresaAndName(usuario.getEmpresa().getIdEmpresa(),
...
//											notificacion.getNotificacion());
							...
							if (smsRestantes.compareTo(0) <= 0) {
								...
								for (Contacto contacto : notificacion.getDestinatarios()) {
									...
									if (notificacion.getCanal().getIdCanal().compareTo(Constants.CANAL_WHATSAPP) == 0) {
										telefono = "whatsapp:" + contacto.getPais().getCodigo()
												+ contacto.getTelefono();
									} else {
										telefono = contacto.getPais().getCodigo() + contacto.getTelefono();
									}
									message = this.smsService.sendSMS(usuario.getEmpresa().getIdEmpresa(), telefono,
											notificacion.getNotificacion());
	...
	public ListenableFuture<ResponseEntity<PaqueteConsumoResponse>> retrieveRemainingNotifications(
		...
		if (input.getIdUsuario() == null || input.getIdUsuario() <= 0) {
			...
			List<PaqueteConsumo> paquetesConsumo = this.paqueteConsumoService
					.findPackagesByCompanyAndStatus(usuario.getEmpresa().getIdEmpresa(), input.getTipo());
			...
			if (paquetesConsumo != null && paquetesConsumo.size() > 0) {
				...
				List<DetallePaquete> detalles = this.detallePaqueteService
						.findByPaquete(paquetesConsumo.get(0).getPaquete().getIdPaquete());

				for (DetallePaquete detalle : detalles) {
					if (detalle.getCanal().getIdCanal().compareTo(Constants.CANAL_SMS) == 0) {
						notificacionesRestantes
								.setCantidadMensajes(detalle.getCuota() - paquetesConsumo.get(0).getConsumoSMS());
					} else if (detalle.getCanal().getIdCanal().compareTo(Constants.CANAL_EMAIL) == 0) {
//						notificacionesRestantes.setCantidadCorreo(-1);
						notificacionesRestantes
								.setCantidadCorreo(detalle.getCuota() - paquetesConsumo.get(0).getConsumoMAIL());
					} else if (detalle.getCanal().getIdCanal().compareTo(Constants.CANAL_WHATSAPP) == 0) {
						notificacionesRestantes
								.setCantidadWhatsapp(detalle.getCuota() - paquetesConsumo.get(0).getConsumoWhatsapp());
					}
				}
	...
	public ListenableFuture<ResponseEntity<DatosNotificacionPrecioResponse>> retrieveNotificationPricePerMonth(
		...
		List<Notificacion> notificaciones = this.notificacionService.findByCompanyAndStatusPayment(
				usuario.getEmpresa().getIdEmpresa(), false, Constants.ESTADO_NOTIFICACION_ENVIADA);

		BigDecimal tarifa = new BigDecimal(this.parametroService
				.findParameterByEmpresaAndName(usuario.getEmpresa().getIdEmpresa(), Constants.NOTIFICATION_RATE)
				.getValor());
	...
	public ListenableFuture<ResponseEntity<?>> obtenerNotificacionPorFechaProgramacion(
		...
		if (input.getFechaInicio() == null) {
			...
			List<com.cycsystems.heymebackend.common.Notificacion> notificaciones = this.notificacionService
					.findByProgrammingDate(fechaInicio, fechaFin, usuario.getEmpresa().getIdEmpresa()).stream()
					.map(CNotificacion::EntityToModel).collect(Collectors.toList());
	...
	public ListenableFuture<ResponseEntity<?>> obtenerNotificacionPorFechaShipping(
		...
		if (input.getFechaInicio() == null) {
			...
			if (usuario == null) {
				...
				List<com.cycsystems.heymebackend.common.Notificacion> notificaciones = this.notificacionService
						.findByShippingDate(fechaInicio, fechaFin, usuario.getEmpresa().getIdEmpresa()).stream()
						.map(CNotificacion::EntityToModel).collect(Collectors.toList());
	...
	public ListenableFuture<ResponseEntity<?>> obtenerNotificacionPorTitulo(@RequestBody NotificacionRequest input) {
		...
		if (input.getNotificacion().getEstado() == null
			...
			if (usuario == null) {
				...
				List<com.cycsystems.heymebackend.common.Notificacion> notificaciones = this.notificacionService
						...
						.stream().map(CNotificacion::EntityToModel).collect(Collectors.toList());
	...
	public ListenableFuture<ResponseEntity<?>> obtenerNotificacionesPorUsuario(@RequestBody NotificacionRequest input) {
		...
		} else if (input.getNotificacion() == null || input.getNotificacion().getEstado() == null
				|| input.getNotificacion().getEstado().getIdEstadoNotificacion() < 0) {
			output.setCodigo(Response.NOTIFICATION_STATUS_NOT_EMPTY.getCodigo());
			output.setDescripcion(Response.NOTIFICATION_STATUS_NOT_EMPTY.getMessage());
			output.setIndicador(Response.NOTIFICATION_STATUS_NOT_EMPTY.getIndicador());
			...
			if (usuario == null) {
				...
				if (input.getNotificacion().getEstado().getIdEstadoNotificacion() == 0) {
					notificaciones = this.notificacionService
							.findByUser(usuario.getEmpresa().getIdEmpresa(), input.getNombreUsuario()).stream()
							.map(CNotificacion::EntityToModel).collect(Collectors.toList());
				} else {
					notificaciones = this.notificacionService
							.findByUser(usuario.getEmpresa().getIdEmpresa(), input.getNombreUsuario(),
									input.getNotificacion().getEstado().getIdEstadoNotificacion())
							.stream().map(CNotificacion::EntityToModel).collect(Collectors.toList());
				}
	...
	@Async
	@PostMapping("/save")
	public ListenableFuture<ResponseEntity<?>> guardarNotificacion(@RequestBody NotificacionRequest input) {
		...
		if (input.getNotificacion().getTitulo() == null || input.getNotificacion().getTitulo().isEmpty()) {
			...
			output.setDescripcion(Response.NOTIFICATION_TITLE_ERROR.getMessage());
			output.setIndicador(Response.NOTIFICATION_TITLE_ERROR.getIndicador());
		} else if (input.getNotificacion().getFechaEnvio() == null) { // ||
																		// input.getNotificacion().getFechaEnvio().before(new
																		// Date())) {
		...
		} else if (input.getNotificacion().getNotificacion() == null
				|| input.getNotificacion().getNotificacion().isEmpty()) {
			output.setCodigo(Response.NOTIFICATION_CONTENT_ERROR.getCodigo());
			output.setDescripcion(Response.NOTIFICATION_CONTENT_ERROR.getMessage());
			output.setIndicador(Response.NOTIFICATION_CONTENT_ERROR.getIndicador());
		} else if (input.getNotificacion().getCanal() == null || input.getNotificacion().getCanal().getIdCanal() == null
